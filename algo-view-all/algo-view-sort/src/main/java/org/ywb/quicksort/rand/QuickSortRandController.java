...
public class QuickSortRandController extends BaseAlgoVisualizer<QuickSortData> {

    private int n;

    public QuickSortRandController(AlgoFrame frame, int n) {
        super(frame);
        this.n = n;
    }
    ...
    public QuickSortData initData() {
        return new QuickSortData(n,super.getFrame().getCanvasHeight(), QuickSortData.Type.Identical);
    }
    ...
    private int partition(QuickSortData data,int l, int r){
        ...
        //随机一个位置的元素作为此次的基准值，然后该值和数组的第一个元素进行交换
        int p = (int) (Math.random() * (r - l + 1)) +l;
        setData(data,l,r,-1,p,-1);
        data.swap(l,p);
